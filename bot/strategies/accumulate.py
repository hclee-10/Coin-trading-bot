...
* `dca_pct`     — 고정 % 기준: 3봉 누적 변화가 1.0% 이상
...
* `dca_10s` — 10초봉 ±0.15% (거래소가 주는 가장 짧은 봉 — '초단위'의 실체)
* `dca_1m`  — 1분봉 ±0.3%
* `dca_5m`  — 5분봉 ±0.6%
* `dca_15m` — 15분봉 ±0.9%
* `dca_1h`  — 1시간봉 ±1.5%

문턱은 √시간 비례(변동성은 대략 시간의 제곱근에 비례)를 따르되, 조용한 장에서도
하루 몇 번은 잡히는 수준으로 맞췄다 — 급변이 영영 안 잡히는 전략은 데이터를
남기지 못해 실험에서 의미가 없다.
...
class DcaPctStrategy(_AccumulateBase):
    summary = "무매도 적립식 ② 고정 % 기준 — 3봉 변화가 1.0%면 급변"
    description = """
급변의 자로 **고정 퍼센트**를 쓰는 변형이다. 3봉 누적 변화가 1.0% 를 넘으면
급변으로 본다. 사람이 차트를 보며 "급등이네"라고 느끼는 직관에 가장 가깝고
기준이 투명한 것이 장점, 시장의 변동성 국면과 무관하게 같은 자를 들이대는
것이 단점이다 — 요동치는 장에서는 1.0% 가 일상이라 적립이 너무 잦아지고,
극도로 조용한 장에서는 급변이 영영 안 잡힐 수 있다. dca_atr 와의 성적 차이가
곧 "적응형 기준의 값어치"다.
""" + _DESCRIPTION_COMMON
    algorithm = """
**급변 판정**  |종가 ÷ 3봉 전 종가 − 1| ≥ 1.0%, 직전 봉 기준으로는 문턱
아래였다가 이번 봉에 넘어선 순간만
...
**파라미터**  `spike_pct`(1.0), `lookback`(3), `max_exposure_pct`(1000)
"""

    def _setup_spike(self) -> None:
        self.spike_pct = float(self.params.get("spike_pct", 1.0))
        self.lookback = int(self.params.get("lookback", 3))
...
_register_horizon("dca_1m", "1m", 0.3, ("1m",),
                  "1분봉 하나가 ±0.3% 움직이면 급변으로 본다.")
_register_horizon("dca_5m", "5m", 0.6, ("5m",),
                  "5분봉 하나가 ±0.6% 움직이면 급변으로 본다.")
_register_horizon("dca_15m", "15m", 0.9, ("15m",),
                  "15분봉 하나가 ±0.9% 움직이면 급변으로 본다.")
_register_horizon("dca_1h", "1h", 1.5, ("1h",),
                  "1시간봉 하나가 ±1.5% 움직이면 급변으로 본다.")
