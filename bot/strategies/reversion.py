...
"""
    algorithm = """
**지표**  RSI(14, Wilder 평활), ATR(14)

**진입**  떨어지는 도중이 아니라 **돌아서는 순간**을 잡는다.
- 롱: 직전 봉 RSI < 30 이고 이번 봉 RSI ≥ 30 (과매도 탈출)
- 숏: 직전 봉 RSI > 70 이고 이번 봉 RSI ≤ 70 (과매수 이탈)

**청산**  RSI 가 50(중립)에 도달하면 청산.

**손절**  진입가 ∓ (ATR14 × 1.5) — 역추세라 추세추종보다 좁게 잡는다.

**확신도**  직전 봉 RSI 가 임계선에서 얼마나 멀었는지
- 15 이상 (RSI 15 이하 / 85 이상) → VERY_HIGH
- 8 이상 → HIGH · 3 이상 → MEDIUM · 그 외 LOW

**파라미터**  `period`, `oversold`, `overbought`, `exit_level`, `atr_multiplier`
"""
...
**확신도**  밴드를 벗어난 깊이 `|밴드 − 직전종가| / 밴드 × 100`
- ≥ 1.0% → VERY_HIGH · ≥ 0.5% → HIGH · ≥ 0.2% → MEDIUM · 그 외 LOW
...
"""
    algorithm = """
**지표**  EMA(50) 을 기준선으로 사용

**진입**  기준선에서 벌어진 **반대 방향**으로 들어간다.
- 롱: 종가가 기준선보다 `step_pct`(기본 1%) 이상 아래
- 숏: 종가가 기준선보다 `step_pct` 이상 위

**청산**  기준선으로 돌아오면 청산 (롱은 이격 ≥ 0, 숏은 ≤ 0).

**손절**  진입가 ∓ 3% (고정). 흔한 그리드 봇과 달리 **손절을 반드시 건다.**

**확신도**  이격 ÷ step_pct = 몇 칸 벌어졌는지
- ≥ 3칸 → VERY_HIGH · ≥ 2칸 → HIGH · ≥ 1.5칸 → MEDIUM · 그 외 LOW

**한 번에 한 포지션만** 잡는다. 여러 층으로 분할 진입하는 방식은 손실이 무한정
커질 수 있어 넣지 않았다.

**파라미터**  `period`, `step_pct`, `stop_pct`
"""
