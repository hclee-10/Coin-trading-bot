...
| **전략 경쟁** | 등록된 전략들이 같은 시세로 동시에 모의매매한 실시간 순위표. 승률 0% 전략 숨기기 필터 |
...
python -m bot strategy dca_atr        # 자세한 설명
```

한때 61종의 지표 전략이 있었으나 전부 폐기했다. 지금은 **급등하면 숏,
급락하면 롱을 고정 금액으로 사 모으기만 하는** 무매도 적립식 하나에 집중하고,
급등/급락의 기준만 세 가지로 나눠 경쟁시킨다:

**자(측정 방식)별 3종** — 기본 시간대(5분봉) 위에서 급변을 다르게 잰다:

| 전략 | 급변 기준 |
|---|---|
| `dca_atr` | 3봉 누적 변화 ≥ ATR14 × 2 — 변동성에 자동 적응 |
| `dca_pct` | 3봉 누적 변화 ≥ 1.5% — 사람 직관에 가까운 고정 자 |
| `dca_channel` | 직전 20봉 최고가 돌파 / 최저가 이탈 — 완만한 일방통행도 잡는 범위 기준 |

**시간 단위별 5종** — 같은 "봉 하나의 등락률" 자를 다른 시간대 봉에 댄다.
각 전략은 자기 시간대 봉을 거래소에서 직접 받으므로 봇의 판단 주기(15초)와
무관하다:

| 전략 | 급변 기준 |
|---|---|
| `dca_10s` | 10초봉 ±0.3% — '초단위'의 실체 (Gate 10초봉, 반응 지연 10~30초) |
| `dca_1m` | 1분봉 ±0.5% |
| `dca_5m` | 5분봉 ±1.0% |
| `dca_15m` | 15분봉 ±1.5% |
| `dca_1h` | 1시간봉 ±2.5% |

짧은 단위일수록 신호가 잦고 수수료가 쌓이며, 긴 단위일수록 드물고 큰 급변만
잡는다. 순위표에서 다섯의 낙폭·수수료·적립 횟수를 비교하면 "어떤 시간 단위의
급변이 되돌아오는 급변인가"가 데이터로 드러난다.

⚠️ 이 계열은 수익 전략이 아니라 **자본 소요를 재는 실험 장치**다. 익절도 손절
청산도 없이 적립만 하므로, 순위표의 미실현 손익·최대 낙폭·청산위험(수수료·
펀딩비 포함)이 "이 방식을 버티는 데 필요한 자금"을 보여 준다. 회당 금액은
대시보드 **거래 설정 패널에서 바로 바꿀 수 있다** (DB에 저장되어 재배포 후에도 유지).

폐기된 전략들은 git 히스토리에 그대로 있다 — 되살리려면
`git log --oneline -- bot/strategies/` 에서 삭제 직전 커밋을 찾아 복원한다.
