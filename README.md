...
| **전략 경쟁** | 27개 전략이 같은 시세로 동시에 모의매매한 실시간 순위표 |
...
`default_stop_loss_pct` 로 채워진다. **익절은 기본으로 걸지 않는다**
(`default_take_profit_pct: 0`) — 수익률에 상한을 두지 않겠다는 뜻이다.
포지션은 전략의 청산 신호나 손절로만 닫힌다.
...
출력은 수익률·최대낙폭·거래횟수·승률·손익비·수수료 순으로 정렬된다.
