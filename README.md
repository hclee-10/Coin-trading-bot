...
| **전략 경쟁** | 10개 전략이 같은 시세로 동시에 모의매매한 실시간 순위표 |
...
| 전략 | 분류 | 요약 |
|---|---|---|
| `ema_cross` | 추세추종 | 빠른 이동평균이 느린 이동평균을 위로 뚫으면 매수 |
| `donchian_breakout` | 추세추종 | 최근 N봉의 최고가를 넘으면 매수 (터틀 트레이딩) |
| `supertrend` | 추세추종 | 변동성으로 만든 추세선을 가격이 넘으면 방향 전환 |
| `macd_trend` | 추세추종 | MACD 히스토그램이 0선을 넘으면 진입 |
| `bollinger_breakout` | 추세추종 | 볼린저밴드 상단을 뚫으면 추세 시작으로 보고 매수 |
| `rsi_reversion` | 평균회귀 | RSI 과매도에서 매수, 과매수에서 매도 |
| `bollinger_reversion` | 평균회귀 | 밴드 밖으로 나간 가격이 안으로 돌아오면 진입 |
| `volatility_breakout` | 돌파 | 직전 봉 변동폭의 k배를 넘어서면 그 방향으로 진입 |
| `trend_pullback` | 조합 | 상승 추세에서 눌릴 때만 매수 |
| `grid` | 횡보 전용 | 기준선에서 벌어지면 반대로 진입 (⚠️ 추세장 위험) |

`bollinger_breakout` 과 `bollinger_reversion` 은 **같은 지표를 정반대로 해석한다.**
둘의 백테스트 성적을 비교하면 이 시장이 추세형인지 횡보형인지 드러난다.
